...
//			}
			...
			if (_map.renderViewport != null && _map.renderViewport != _map.gameObject) {
				...
				EditorGUILayout.EndHorizontal ();
				if (_map.renderViewportMode == WorldMap2D.ViewportMode.Viewport3D) {
					EditorGUILayout.BeginHorizontal();
...
//												labelStyle = _map.showCountryNames || _map.showFrontiers ? sectionHeaderBoldStyle : sectionHeaderNormalStyle;
			...
			if (expandInteractionSection) {
                ...
                if (_map.respectOtherUI) {
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label(new GUIContent("   Blocking Mask", "This option let you specify which UI elements can block the interaction"), GUILayout.Width(120));
                    _map.blockingMask = LayerMaskField(_map.blockingMask);
                    EditorGUILayout.EndHorizontal();
                }

                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("Pointer Source", GUILayout.Width(120));
                _map.pointerSource = (PointerSource)EditorGUILayout.EnumPopup(_map.pointerSource);
                EditorGUILayout.EndHorizontal();
