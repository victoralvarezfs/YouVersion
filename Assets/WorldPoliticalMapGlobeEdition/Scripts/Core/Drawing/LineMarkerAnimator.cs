...
namespace WPM {
    ...
    public class LineMarkerAnimator : MonoBehaviour {
        ...
        /// <summary>
        /// Use a 3D tube instead of a line
        /// </summary>
        public bool useTube;
        ...
        /// <summary>
        /// The underline Tube Renderer component
        /// </summary>
        ...
        void Start() {
            ...
            if (useTube) {
                _tr = transform.GetComponent<TubeRenderer>();
                if (_tr == null) {
                    _tr = gameObject.AddComponent<TubeRenderer>();
                }
                _tr.useWorldSpace = false;
                _tr.startWidth = lineWidth;
                _tr.endWidth = lineWidth;
                _tr.material = lineMaterial; // needs to instantiate to preserve individual color so can't use sharedMaterial
            } else {
        ...
        void SetProgress(float t) {
            ...
            if (t <= 0) {
                ...
                SetPositionCount(vertices.Length);
                for (int k = 0; k < vertices.Length; k++) {
                    SetPosition(k, vertices[k]);
                }
        ...
        void SetPositionCount(int count) {
            if (useTube) _tr.positionCount = count; else _lr.positionCount = count;
        }

        void SetPosition(int index, Vector3 position) {
            if (useTube) _tr.SetPosition(index, position); else _lr.SetPosition(index, position);
        }
    }
}