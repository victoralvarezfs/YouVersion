...
namespace WPMF {
    ...
    public partial class WorldMap2D : MonoBehaviour {
        ...
        bool ValidCountryIndex(int countryIndex) {
            return countryIndex >= 0 && countries != null && countryIndex < countries.Length;
        }

        bool ValidCountryRegionIndex(int countryIndex, int regionIndex) {
            return countryIndex >= 0 && countries != null && countryIndex < countries.Length && regionIndex >= 0 && countries[countryIndex].regions != null && regionIndex < countries[countryIndex].regions.Count;
        }
        ...
        public void HideCountryRegionSurface(int countryIndex, int regionIndex) {
            bool releaseCustomMaterial = true;
            if (_countryHighlightedIndex != countryIndex || _countryRegionHighlightedIndex != regionIndex) {
                ...
                if (surfaces.ContainsKey(cacheIndex)) {
                    if (surfaces[cacheIndex] == null) {
                        ...
                        releaseCustomMaterial = false;
                    }
                }
            }
            if (releaseCustomMaterial) {
        ...
        public void BlinkCountry(int countryIndex, Color color1, Color color2, float duration, float blinkingSpeed) {
            ...
            if (countries[countryIndex].regions == null) return;
            int regionsCount = countries[countryIndex].regions.Count;
            for (int k = 0; k < regionsCount; k++) {
                BlinkCountry(countryIndex, k, color1, color2, duration, blinkingSpeed);
            }
        }
        ...
        /// Flashes specified country by index in the countries collection.
        /// </summary>
        public void BlinkCountryMainRegion(int countryIndex, Color color1, Color color2, float duration, float blinkingSpeed) {
            if (!ValidCountryIndex(countryIndex)) return;
