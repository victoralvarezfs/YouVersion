			...
			for (int k = 0; k < decorators.Count; k++) {
				...
				if (countryIndex >= 0) {
					...
					if (active) {
						...
						// label Font override (text mesh pro)
						if (country.labelFontTMProOverride != decorator.labelFontTMProOverride) {
							country.labelFontTMProOverride = decorator.labelFontTMProOverride;
							needsLabelRedraw = true;
						}
						if (country.labelFontTMProMaterialOverride != decorator.labelFontTMProMaterialOverride) {
							country.labelFontTMProMaterialOverride = decorator.labelFontTMProMaterialOverride;
							needsLabelRedraw = true;
						}
						...
						// size override
						if (country.labelFontSizeOverride != decorator.labelOverridesSize) {
							country.labelFontSizeOverride = decorator.labelOverridesSize;
							needsLabelRedraw = true;
						}
						if (country.labelFontSizeOverride && country.labelFontSize != decorator.labelSize) {
							country.labelFontSize = decorator.labelSize;
							needsLabelRedraw = true;
						}
						// label visible
						...
						if ((country.customLabel == null && decorator.customLabel.Length > 0) || (country.customLabel != null && country.customLabel != decorator.customLabel)) {
							...
							if (country.labelTextGameObject != null) {
								DestroyImmediate (country.labelTextGameObject);
							}
							needsLabelRedraw = true;
						}
						// colorize
