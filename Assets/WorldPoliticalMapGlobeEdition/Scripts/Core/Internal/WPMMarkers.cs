...
namespace WPM {
    ...
    public partial class WorldMapGlobe : MonoBehaviour {
        ...
        const string MARKERS_LAYER_NAME = "Markers";
        ...
        void CheckMarkersLayer () {
            ...
            Transform t = transform.Find(MARKERS_LAYER_NAME);
            if (t != null) {
                markersLayer = t.gameObject;
                return;
            }
            markersLayer = new GameObject(MARKERS_LAYER_NAME);
        ...
        void mAddMarker (GameObject marker, Vector3 sphereLocation, float markerScale, bool isBillboard, float surfaceOffset, bool baselineAtBottom, bool preserveOriginalRotation = true) {
            ...
            if (baselineAtBottom) {
                markerTransform.TryGetComponent<MeshFilter>(out var meshFilter);
                if (meshFilter != null)
                    height = meshFilter.sharedMesh.bounds.size.y;
                else {
                    markerTransform.TryGetComponent<Collider>(out var collider);
                    if (collider != null) {
            ...
            if (markerTransform != markersLayer.transform) {
                markerTransform.SetParent(markersLayer.transform, false);
            }
            markerTransform.localPosition = _earthInvertedMode ? sphereLocation * (1.0f - h) : sphereLocation * (1.0f + h * 0.5f);

            // apply custom scale
            markerTransform.localScale = Misc.Vector3one * markerScale;

            Quaternion originalRotation = marker.transform.localRotation;

            if (_earthInvertedMode) {
                // flip localscale.x
                markerTransform.localScale = new Vector3(-markerTransform.localScale.x, markerTransform.localScale.y, markerTransform.localScale.z);
                // once the marker is on the surface, rotate it so it looks to the surface
                markerTransform.LookAt(transform.position, transform.up);
                if (!isBillboard)
                    markerTransform.Rotate(new Vector3(90, 0, 0), Space.Self);
            ...
            if (preserveOriginalRotation) {
                markerTransform.Rotate(originalRotation.eulerAngles, Space.Self);
            }
