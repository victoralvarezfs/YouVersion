...
namespace WPM {
	...
	public class WorldMapTicker : MonoBehaviour {
		...
		void OnDestroy () {
			GameObject overlayLayer = map.GetOverlayLayer (false, false);
		...
		public void Init () {
			...
			if (defaultFont == null) {
				defaultFont = Instantiate (Resources.Load <Font> ("Font/Lato"));
		...
		public bool UpdateTickerBands () {
			...
			if (tickerBaseLayer == null) {
				changes = true;
				GameObject overlayLayer = map.GetOverlayLayer (true, true);
				if (overlayLayer == null)
					return changes;
				Transform t = overlayLayer.transform.Find (TICKERS_LAYER);
				if (t == null) {
