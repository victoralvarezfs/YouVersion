...
namespace WPM {
    ...
    public partial class WorldMapGlobe : MonoBehaviour {
        ...
        const string TROPIC_LINES_LAYER = "TropicLines";

        GameObject cursorLayer, cursorVertical, cursorHorizontal;
        GameObject latitudeLayer, longitudeLayer, tropicLayer;
        Material cursorMat, gridMatOverlay, gridMatMasked;
        Material tropicMatOverlay, tropicMatMasked;
        ...
        void DrawLatitudeLines() {
            ...
            latitudeLayer.transform.localRotation = Misc.QuaternionZero;
        ...
        void DrawTropicLines() {
            ...
            float[] tropicsLatitudes = new float[] { -23.4363f, 23.4363f };
            ...
            for (int a = 0; a < 2; a++) {
                for (int h = 1; h >= -1; h--) {
                    ...
                    float angle = tropicsLatitudes[a] * Mathf.Deg2Rad;
                    float y = h * Mathf.Sin(angle) * r;
                    float r2 = Mathf.Cos(angle) * r;
            ...
            Transform t = transform.Find(TROPIC_LINES_LAYER);
            if (t != null)
                DestroyImmediate(t.gameObject);
            tropicLayer = new GameObject(TROPIC_LINES_LAYER);
            tropicLayer.transform.SetParent(transform, false);
            tropicLayer.layer = gameObject.layer;
            tropicLayer.transform.localPosition = Misc.Vector3zero;
            tropicLayer.transform.localRotation = Misc.QuaternionZero;
            ...
            MeshFilter mf = tropicLayer.AddComponent<MeshFilter>();
            mf.sharedMesh = mesh;

            MeshRenderer mr = tropicLayer.AddComponent<MeshRenderer>();
            mr.receiveShadows = false;
            mr.reflectionProbeUsage = UnityEngine.Rendering.ReflectionProbeUsage.Off;
            mr.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
            mr.sharedMaterial = _gridMode == GRID_MODE.OVERLAY ? tropicMatOverlay : tropicMatMasked;
